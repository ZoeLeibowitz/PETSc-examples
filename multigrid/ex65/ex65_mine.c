...
static PetscErrorCode MyDMShellCreate(MPI_Comm comm, DM da, PetscInt level, DM *shell)
{
...
static PetscErrorCode Coarsen(DM shell, MPI_Comm comm, DM *dmnew)
{
  ...
  PetscCall(DMGetApplicationContext(shell, (void **)&level));
  PetscCall(MyDMShellCreate(PetscObjectComm((PetscObject)shell), dacoarse, level-1, dmnew));
  PetscFunctionReturn(PETSC_SUCCESS);
}
